...
const Project = ({ slug }) => {
  ...
  return (
    <div
      className={`relative w-full min-h-[calc(100vh-112px)] flex flex-col justify-between items-center pt-8 pb-16 px-8 md:px-20 2xl:container 2xl:mx-auto bg-primary`}
    >
        ...
        <div className="w-full flex items-center text-base space-x-4 text-secondary">
          {traceRoute.map((trace, i) => {
            if (i == traceRoute.length - 1) {
              return <div key={i} className="no-underline">{trace}</div>;
            } else {
              return (
      ...
      <div className="w-full md:w-8/12 md:mx-auto flex flex-col items-center bg-primary pt-6">
        ...
        <div className="w-full h-auto mx-auto flex justify-between items-center mb-4 text-secondary">
          <h3 className={`font-light text-sm font-poppins ${isLoading && 'w-3/4 h-5 rounded-md bg-secondary/30 animate-pulse'}`}>
            {selectedProject && selectedProject.descriptions}
          </h3>
        </div>
        <div className="w-full mx-auto flex justify-between items-center mb-6">
          <div className={`relative h-44 sm:h-64 md:h-96 w-full flex justify-center rounded-lg overflow-hidden ${isLoading ? 'bg-secondary/30 animate-pulse' : 'bg-tertiary'}`}>
            {selectedProject && (
              <Image
                loader={myLoader}
        ...
        <div className={`w-full h-auto mx-auto flex flex-wrap justify-start items-center mb-6 border-b border-secondary/50 pb-3 md:pb-6 ${isLoading && 'h-3 rounded-md bg-secondary/30 animate-pulse'}`}>
          {selectedProject &&
